...
[r c] = size(image_paths);

stepSize = 4;
binSize = 8;

image_feats = zeros(r, vocab_size);
individualSampleSize = 500;
...
for i = 1:r
    ...
    [~, SIFT_features] = vl_dsift(single(im), 'Fast', 'Step', stepSize, 'Size', binSize);
    
    % sample set amount of SIFT Features
    
    SIFT_features = datasample(SIFT_features', individualSampleSize)';
    
    [sfR sfC] = size(SIFT_features);
    
    D = vl_alldist2(single(SIFT_features), single(vocab));
    
    % get minimim of each row     
    [~, I] = min(D,[],2);
    %      
    [wordFreq, wordInd] = hist(I, unique(I));
    image_feats(i, wordInd) = wordFreq/norm(wordFreq);
