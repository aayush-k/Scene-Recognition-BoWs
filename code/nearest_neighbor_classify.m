...
[numTests, dim] = size(test_image_feats);
...
%   sort by distance 
    distances = D(i,:);
    [sortVal,sortInd] = sort(distances);
%   get k nearest neighbors' indices in the training data features and
%   labels
    kNNInd = sortInd(1:k);
    kNNLabels = train_labels(kNNInd);
    
    candidateLabels = unique(kNNLabels);
    maxNNInd = 0;
    maxNNVal = 0;
    
    for j = 1:length(candidateLabels)
       clVal = sum(strcmp(candidateLabels{j}, kNNLabels));
       if clVal > maxNNVal
           maxNNVal = clVal;
           maxNNInd = j;
       end
    ...
    label = candidateLabels(maxNNInd);
